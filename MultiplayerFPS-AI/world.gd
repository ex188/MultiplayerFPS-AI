...
func _unhandled_input(event):
	...
	# Test AI damage with Space key
	if Input.is_action_just_pressed("ui_accept"):  # Space key
		print("Testing AI damage...")
		for ai in ai_players:
			if ai and is_instance_valid(ai):
				ai.test_damage()
				break  # Only test first AI
	
	# Test all AI damage with Shift key
	if Input.is_action_just_pressed("ui_text_submit"):  # Shift key
		print("Testing all AI damage...")
		for i in range(ai_players.size()):
			var ai = ai_players[i]
			if ai and is_instance_valid(ai):
				print("Testing damage on AI ", i)
				ai.test_damage()
				await get_tree().create_timer(0.5).timeout  # Wait between tests
	
	# Test raycast with Enter key
	if Input.is_action_just_pressed("ui_select"):  # Enter key
		print("Testing raycast hit...")
		for ai in ai_players:
			if ai and is_instance_valid(ai):
				ai.test_raycast_hit()
				break  # Only test first AI
	
	# Test all AI collision with Tab key
	if Input.is_action_just_pressed("ui_cancel"):  # Tab key
		print("=== Testing all AI collision ===")
		for i in range(ai_players.size()):
			var ai = ai_players[i]
			if ai and is_instance_valid(ai):
				print("AI ", i, " - collision_layer: ", ai.collision_layer, " collision_mask: ", ai.collision_mask)
				var collision_shape = ai.get_node("CollisionShape3D")
				if collision_shape:
					print("AI ", i, " - collision shape disabled: ", collision_shape.disabled)
				else:
					print("AI ", i, " - ERROR: No collision shape!")
			else:
				print("AI ", i, " - ERROR: Invalid AI!")
	
	# Test AI shooting with T key
	if Input.is_key_pressed(KEY_T):
		print("Testing AI shooting...")
		for ai in ai_players:
			if ai and is_instance_valid(ai):
				ai.test_shoot()
				break  # Only test first AI
...
func _process(delta):
	...
	# Update AI spawn timer for respawning dead AI players
	ai_spawn_timer += delta
	
	# Only respawn AI players if they died, don't spawn extra ones
	if ai_spawn_timer >= ai_spawn_delay and ai_players.size() < max_ai_players:
		# Check if any AI players are actually dead/missing
		var dead_ai_count = 0
		for ai in ai_players:
			if not is_instance_valid(ai) or ai.is_queued_for_deletion():
				dead_ai_count += 1
		
		if dead_ai_count > 0:
			_spawn_single_ai_player()
			ai_spawn_timer = 0.0
			print("Respawning dead AI. Current count: ", ai_players.size(), " / ", max_ai_players)
